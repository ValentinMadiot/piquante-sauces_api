...
export class SaucesService {
  ...
  private apiUrl = environment.apiUrl; // Utilisation de l'URL d'API depuis l'environnement

  constructor(private http: HttpClient, private auth: AuthService) {}

  getSauces() {
    this.http
      .get<Sauce[]>(`${this.apiUrl}/sauces`)
      .pipe(
  ...
  getSauceById(id: string) {
    return this.http
      .get<Sauce>(`${this.apiUrl}/sauces/${id}`)
      .pipe(catchError((error) => throwError(error.error.message)));
  }

  likeSauce(id: string, like: boolean) {
    return this.http
      .post<{ message: string }>(`${this.apiUrl}/sauces/${id}/like`, {
  ...
  createSauce(sauce: Sauce, image: File) {
    ...
    return this.http
      .post<{ message: string }>(`${this.apiUrl}/sauces`, formData)
      .pipe(catchError((error) => throwError(error.error.message)));
  }
  ...
  modifySauce(id: string, sauce: Sauce, image: string | File) {
    if (typeof image === 'string') {
      return this.http
        .put<{ message: string }>(`${this.apiUrl}/sauces/${id}`, sauce)
        .pipe(catchError((error) => throwError(error.error.message)));
    } else {
      const formData = new FormData();
      formData.append('sauce', JSON.stringify(sauce));
      formData.append('image', image);
      return this.http
        .put<{ message: string }>(`${this.apiUrl}/sauces/${id}`, formData)
        .pipe(catchError((error) => throwError(error.error.message)));
    }
  }
  ...
  deleteSauce(id: string) {
    return this.http
      .delete<{ message: string }>(`${this.apiUrl}/sauces/${id}`)
      .pipe(catchError((error) => throwError(error.error.message)));
  }
}
